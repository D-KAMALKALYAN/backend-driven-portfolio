...
import type { ActiveResume } from '../types/rows';
 ...
 *
 */
export function useActiveResume(): { resume: ActiveResume | null; loading: boolean; error: string | null } {
  ...
  return {
    ...
    error: error ? (errorMessage(error) ?? 'Could not load resume') : null,
  };
}
