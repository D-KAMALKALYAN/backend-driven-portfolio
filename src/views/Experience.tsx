...
// tech_used is text[] in the schema. The string branch stays for rows edited
...
function parseAchievements(exp: ExperienceRow): string[] {
...
function ExperienceDrawer({ exp, onClose }: { exp: ExperienceRow; onClose: () => void }) {
...
interface TimelineItemProps {
  ...
  index: number;
  isLast: boolean;
  onSelect: (exp: ExperienceRow) => void;
}

function TimelineItem({ exp, index, isLast, onSelect }: TimelineItemProps) {
  const start    = fmtDate(exp.start_date);
  const end      = exp.end_date ? fmtDate(exp.end_date) : 'Present';
  const techs    = parseTechs(exp.tech_used);
  const isCurrent = !exp.end_date;
  ...
  return (
    <div className="enter relative flex gap-6 group" style={enterAt(index * 100)}>
      {/* Timeline column */}
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function Experience({ experience }: { experience: ExperienceRow[] }) {
