...
export interface ProjectDetailProps {
  ...
  storytelling: ProjectStorytelling[];
}
...
function StorytellingSection({ rows }: { rows: ProjectStorytelling[] | null | undefined }) {
...
// `content` is jsonb with a shape that depends on `type`. Each branch narrows
// the fields it needs; an unexpected shape degrades to its JSON text rather
// than throwing inside the render tree.
function renderContent(section: ProjectSection): ReactNode {
  const c = asObject(section.content) ?? {};
  switch (section.type) {
    case 'text': {
    ...
    case 'gallery': {
      // Either bare URL strings or { url, caption } objects.
      const raw = c['images'];
      const images = (Array.isArray(raw) ? raw : [])
        .map((img, i) => {
          if (typeof img === 'string') return { url: img, alt: `Gallery ${i + 1}` };
          const o = asObject(img);
          return o ? { url: asString(o['url']), alt: asString(o['caption'], `Gallery ${i + 1}`) } : null;
        })
        .filter((img): img is { url: string; alt: string } => Boolean(img?.url));
      return images.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {images.map((img, i) => <img key={i} src={img.url} alt={img.alt} className="w-full aspect-video rounded-xl object-cover" />)}
        </div>
      ) : null;
    }
    default: {
...
function ProjectLink({ href, icon, label, primary = false }: { href: string | null; icon: ReactNode; label: string; primary?: boolean }) {
  if (!href) return null;
  return (
    <a
      href={href}
...
export default function ProjectDetail({ project, sections, storytelling }: ProjectDetailProps) {
  /* ── Fire project_view event once per project load ── */
  const trackedProjectRef = useRef<string | null>(null);
  useEffect(() => {
    if (trackedProjectRef.current !== project.id) {
  ...
  const viewLabel   = formatViews(viewCount);
      ...
      <Section>
        <Container>
          ...
          {sectionList.length > 0 ? (
            <div className="space-y-4">
              {sectionList.map((s, i) => {
                ...
                return (
                  <div key={s.id} className="enter" style={enterAt(i * 70)}>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-10">
            <Button as={Link} href="/projects" variant="ghost" size="sm">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
