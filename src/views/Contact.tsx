...
const INIT: ContactForm = { name: '', email: '', subject: '', message: '' };
...
function useFocusStyle(hasError: boolean) {
...
export default function Contact() {
  ...
  // Honeypot: hidden from people, filled by bots. Sent as `website`.
  ...
  // contact.availability shape: { items: [{ label, available }] }
  const storedAvailability = asObjectArray(getItems(content, 'contact.availability'))
    .map((item) => ({ label: asString(item['label']), available: item['available'] === true }))
  ...
  const onChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name !== 'subject' && errors[name]) {
      setErrors((p) => { const n = { ...p }; delete n[name]; return n; });
    }
  };

  // The browser no longer writes to the database. The message goes to
  // /api/contact, which validates with the same rules as below (so the
  // client cannot be the more permissive side), records the real IP, stores
  // the row and emails the owner. Client-side validation stays for instant
  ...
  const onSubmit = useCallback(async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload = (await res.json().catch(() => ({}))) as { ok?: boolean; message?: string; errors?: ContactFormErrors };
      if (res.ok && payload.ok) {
        setStatus('success'); setForm(INIT); setErrors({});
      } else if (res.status === 400 && payload.errors) {
        setStatus('idle'); setErrors(payload.errors);
      } else {
        setStatus('error'); setErr(payload.message || errorMsg);
      }
    } catch {
      setStatus('error'); setErr(errorMsg);
    }
  }, [form, honeypot, errorMsg]);
            ...
            <div className="lg:col-span-3">
              ...
              <form onSubmit={onSubmit} noValidate id="contact-form" className="space-y-5">
                ...
                <div aria-hidden="true" className="absolute -left-[9999px] top-0 h-0 w-0 overflow-hidden">
                  ...
                  <input
                    id="contact-website" type="text" name="website" tabIndex={-1} autoComplete="off"
