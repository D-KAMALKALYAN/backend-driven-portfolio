...
/**
 * A platform's own mark, by name (ADR-065).
 *
 * These lived inside `views/Profiles.tsx`, so /profiles showed GitHub's
 * logo while /contact showed the letters "GH". One registry, read by both:
 * a platform added to `external_profiles` is drawn the same way wherever
 * it appears, and an unknown one falls back to a link glyph rather than to
 * nothing.
 *
 * Brand marks are single-path SVGs drawn in `currentColor`, so they take
 * the colour of whatever they sit in and cost nothing beyond the markup.
 */
const ICONS: Record<string, ReactElement> = {
  github: (
    <svg className="w-full h-full" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
...
/** Letters, punctuation and case are noise in a platform name: "GeeksforGeeks" and "geeks for geeks" are one key. */
export function platformKey(platform: string | null | undefined): string {
  return (platform || '').toLowerCase().replace(/[^a-z]/g, '');
}

export function platformIcon(platform: string | null | undefined): ReactElement {
  return ICONS[platformKey(platform)] ?? FALLBACK;
}

/** A sized box around the mark, so callers set one number rather than editing every path. */
export default function PlatformIcon({ platform, className = 'w-5 h-5' }: { platform: string | null | undefined; className?: string }) {
  return <span className={`inline-block shrink-0 ${className}`}>{platformIcon(platform)}</span>;
}

/** A stored URL may have no scheme; a link without one resolves against the current path. */
