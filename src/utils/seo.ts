 ...
 *
 * `siteTitle` comes from site_content seo.title.
 */
export function buildTitle(pathname: string | null | undefined, siteTitle: string | null | undefined): string {
  const base = siteTitle || 'Portfolio';
  if (pathname?.startsWith('/projects/')) return `Projects · ${base}`;
  if (pathname === '/') return base;
  const page = pathname ? ROUTE_TITLES[pathname] : null;
