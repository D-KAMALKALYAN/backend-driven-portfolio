...
/** What the feed reads from an analytics row. `fetchRecentEvents` selects a
 *  subset of columns, so this is narrower than the full Row. */
export type FeedRow = Partial<Pick<AnalyticsEvent, 'id' | 'event' | 'path' | 'session_id' | 'created_at'>>;
...
function timeOf(row: FeedRow): number | null {
  const t = row?.created_at ? new Date(row.created_at).getTime() : NaN;
  return Number.isNaN(t) ? null : t;
}

/** `rows` are analytics rows, newest first. */
export function groupEventsByVisit(rows: ReadonlyArray<FeedRow | null | undefined> | null | undefined): VisitGroup[] {
 ...
 */
export function formatEventTime(at: number | string | Date | null | undefined): string {
