...
describe('section registry', () => {
  ...
  it('knows exactly the types that have components', () => {
    expect(KNOWN_SECTION_TYPES.sort()).toEqual(['code', 'diagram', 'endorsements', 'explore', 'now', 'prose', 'qa', 'steps', 'table', 'timeline', 'ventures']);
  });
  ...
  it('renders a post body from blocks with the same content types, skipping unknown ones', () => {
    ...
    const { nodes, unknown } = renderBlocks([b('prose', 1), b('code', 2), b('video', 3), b('table', 4), b('qa', 5)]);
