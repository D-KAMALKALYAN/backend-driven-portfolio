...
import { filterProjects, collectFacets, hasActiveFilters, type FilterableProject } from '../utils/projectFilter';
...
const ids = (rows: ReadonlyArray<{ id: string }>) => rows.map((r) => r.id).sort();
...
describe('filterProjects', () => {
  ...
  it('accepts a comma-separated string where an array is expected', () => {
    ...
    // A string where the schema says text[] is the tolerated-at-runtime case,
    // so the fixture is cast past the column type on purpose.
    const odd = [{ id: 'a', title: 'A', tech_stack: 'React, Node', tags: '' }] as unknown as Array<FilterableProject & { id: string }>;
...
describe('collectFacets', () => {
  ...
  it('sorts by count then alphabetically, so the list is stable', () => {
    ...
    for (let i = 1; i < facets.length; i++) {
      ...
      const cur = facets[i];
      if (!prev || !cur) throw new Error('unreachable: loop bounds');
