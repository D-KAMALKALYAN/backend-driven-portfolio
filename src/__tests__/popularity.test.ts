...
const P = (id: string, view_count: number) => ({ id, view_count });
...
describe('getPopularProjectIds', () => {
  ...
  it('handles empty and malformed input', () => {
    ...
    // Rows without an id are exactly what the signature forbids; the runtime
    // guard is what is under test here.
    const malformed = [null, {}, { view_count: 5 }] as unknown as Parameters<typeof getPopularProjectIds>[0];
    expect(getPopularProjectIds(malformed).size).toBe(0);
  });
