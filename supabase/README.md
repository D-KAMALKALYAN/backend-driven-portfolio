...
## Writes are server-only (since `20260913090000`)

The anon role has no INSERT policy on any table. `/api/track` and `/api/contact`
write with the service role; `GET /api/health` shows whether a deployment has it.
A new write path is a route handler plus a test, never a policy.
